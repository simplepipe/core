...
static void __pass_texture_droid_free(const struct ref *ref);
static struct pass_texture_droid *__pass_texture_droid_init(
        struct pass_texture_droid *p, const unsigned len,
        const unsigned width, const unsigned height,
        const unsigned depth, const unsigned stencil,
        const unsigned msaa);
static void __pass_texture_droid_release(struct pass_texture_droid *p);
static void __pass_texture_droid_end(struct pass *p);
        ...
        const unsigned depth,const  unsigned stencil)
{
        ...
        if(samples == 0) {
                ...
                pl = __pass_texture_droid_init(
                        malloc(sizeof(struct pass_texture_droid)),
                ...
                if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
                        __pass_texture_droid_release(pl);
...
struct pass_texture_droid *__pass_texture_droid_init(
        struct pass_texture_droid *p, const unsigned len,
        const unsigned width, const unsigned height,
        const unsigned depth, const unsigned stencil,
        const unsigned msaa)
{
        ...
        p->base.base.base = (struct ref){__pass_texture_droid_free, 1};
        p->render_buffers_sampled = calloc(len, sizeof(unsigned));
        p->render_buffers_len = len;
        p->base.base.end = __pass_texture_droid_end;
        return p;
}

void __pass_texture_droid_release(struct pass_texture_droid *p)
{
...
static void __pass_texture_droid_free(const struct ref *ref)
{
        ...
        struct pass_texture_droid *ptl =
                cast(pt, struct pass_texture_droid, base);

        __pass_texture_droid_release(ptl);
        pass_texture_release(pt);
        pass_release(p);
